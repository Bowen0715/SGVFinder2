...
def get_sample_map(
    ...
    ########ADDED BY AYA FEB 2020 to overcome giant jsdel sizes
    splts = delta_fname.split('.')
    multi_jsdels = splts[0] + '_1.' + splts[1]
    if exists(multi_jsdels):
        delta = []
        for num in range(1,12):
            title = f'{splts[0]}_{num}.{splts[1]}'
            with gzip.open(title) as inf:
                average_read_length, delta_sub = ujson.load(inf)
            delta = delta + delta_sub
    ########ADDED BY AYA FEB 2020 to overcome giant jsdel sizes

    else:
        with gzip.open(delta_fname) as inf:
            average_read_length, delta = ujson.load(inf)
    ...
    for _, mapngs in delta:
        for dest_id, pos1, pos2, used_koef, _ in mapngs:
            ...
            # if ind1 >= len(bacid_maps[dest_id]):
            #     ind1 = len(bacid_maps) - 1
...
def calculate_by_other(old_deldf_p, old_sgvdf_p, old_frames_df, samp_to_map, real_del_thresh, dense_perc, weboutputdir,
    ...
    for col in old_sgvdf.columns:
        ...
        if cur_bac != prev_bac:
            ...
            if exists(join(old_frames_df, cur_bac + '.df')):
                ...
                continue
        covregions.append(
            vsgv_get_region_val([(int(st.split('_')[0]), int(st.split('_')[1])) for st in col.split(':')[1].split(';')], \
    ...
    return concat(covregions, axis=1, sort = True) if len(covregions) > 0 else DataFrame(), \
           concat(delsregions, axis=1, sort = True) if len(delsregions) > 0 else DataFrame()


def collect_from_glob(delta_glob, q, dist_reads, x_coverage, average_read_length, lengthdbp, rate_param, cutoff, outp):
    collect_from_flist(glob(delta_glob), q, dist_reads, x_coverage, average_read_length, lengthdbp, rate_param, cutoff,
                       outp)
...
def work_on_collection(
    ...
    if browser_path is not None:
        try:
            ...
            log_.warning('Could not load taxonomy file (db_path + \'.taxonomy.df\'.')
            taxonomy, genepos = None, None
    bac_samps_map = defaultdict(dict)
    for samp, bacid_maps in samp_to_map.items():
        for bacname, bacmap in bacid_maps.items():
            bac_samps_map[bacname][samp] = bacmap
    ...
    for bacname, bacdict in bac_samps_map.items():
        bacdf = DataFrame(bacdict).T
        #bacdf.to_pickle(f'/ifs/scratch/tk2829_gp/ab4966/SGV/CNIO/sv_find/bacdfs/{bacname}.df')
        ...
        if frames_path is not None:
            bacdf.to_pickle(join(frames_path, bacname + '.df'))
    return (concat(sgvregions_all, axis=1, sort = True) if len(sgvregions_all) > 0 else DataFrame(),
            concat(delsregions_all, axis=1, sort = True) if len(delsregions_all) > 0 else DataFrame())
...
def find_sgvs(bacdf, max_spacing, dense_perc, bacname, deldf, real_del_thresh, clip_quantile,
    ...
    print('doing var regions XXXXXXX')
...
def find_deletions(df_in, bacname, dichotomize, dichotomize_thresh, detectthresh,
    ...
    def ext_cret(s0, s1, snew0, snew1):
        ...
        smin = min(s0, s1, snew0, snew1)
        smax = max(s0, s1, snew0, snew1)
        return (np.average([_cooc_dissim(sx, sy) for sx, sy in product(sX, sY)]) <= cooc_thresh) \
               and getpercdel(smin, smax) >= detectthresh and getpercret(smin, smax) >= detectthresh

    seeds = (deltn.var(0) > cutoffvar).astype(int)
    delsregions = []
    stretches = get_extended_stretches(seeds, df.shape[1], max_spacing, ext_cret)
    sclusters = cluster_stretches(stretches, deltn, _cooc_dissim, cooc_thresh, 'complete')
    for s in sorted(sclusters, key=lambda x: x[0][0]):
        if dichotomize:
            ...
            delsregions.append(extract_scluster(deltn, s).mean(1).to_frame("{}:{}".format(bacname, scluster_to_txt(s))))
...
def extract_scluster(df, scluster):
    return concat([df.loc[:, s[0]:s[1] - 1] for s in scluster], axis=1, sort = True)
    ...
    while True:
        ...
        for i, s in enumerate(stretches):
            ...
            while (i == len(stretches) - 1 or s[1] < stretches[i + 1][0]) and s[1] < dfs1 - 1 \
                    and extend_func(s[0], s[1], s[1], s[1] + 1):
                s = (s[0], s[1] + 1)
...
def _stretch_average(distance, stretches, indices):
    ...
    return np.array(
        [np.average(np.ravel(distmat.loc[si[0]:si[1] - 1, sj[0]:sj[1] - 1])) for si, sj in combinations(stretches, 2)])


def cluster_stretches(stretches, df, unite_func, cutoff_dist, linkage_method):
    if len(stretches) == 0:
        return []
    if len(stretches) == 1:
        return [(stretches[0],)]
    # linkage_method = 'average'  # only average is implemented here
    stretchdf = concat([df.loc[:, s[0]:s[1] - 1] for s in stretches], axis=1, sort = True)
...
def draw_one_region(bacname, binsize, taxonomy, normdf,
    ...
    if len(sgvregions) > 0:
        for clust in concat(sgvregions, axis=1, sort = True):
    ...
    if p3 is not None:
        ...
        if len(delregions) > 0:
            for clust in concat(delregions, axis=1, sort = True):
                for reg in clust.split(':')[1].split(';'):
